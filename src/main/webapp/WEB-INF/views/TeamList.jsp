	...
	<c:import url="/Menu"/>
			<div class="panel panel-success">
				<h3 class="panel-title">
					<div align="left"><b>Team List</b></div>
				</h3>
			</div>
						...
						<tbody>
							<c:forEach items="${ItemList.pageList}" var="item">
								<tr>
									...
									<th>
										<c:forEach var="member" items="${item.users}">
											<c:out value="${member.username}"/><br>
										</c:forEach>
									</th>
									<th><a href="<%=request.getContextPath()%>/Team/edit?id=<c:out value='${item.id}'/>">Edit</a></th>
									<th><a href="<%=request.getContextPath()%>/Team/delete?id=<c:out value='${item.id}'/>">Delete</a></th>
								</tr>
							</c:forEach>
						</tbody>
					</table>
